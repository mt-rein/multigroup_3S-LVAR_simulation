...
do_sim2 <- function(pos, cond, outputfile, verbose = FALSE) {
  ...
  phimat_g1 <- matrix(
    c(phi11_g1_pop, phi12_g1_pop, phi21_g1_pop, phi22_g1_pop),
    ncol = 2,
    byrow = TRUE
  )
  phimat_g2 <- matrix(
    c(phi11_g2_pop, phi12_g2_pop, phi21_g2_pop, phi22_g2_pop),
    ncol = 2,
    byrow = TRUE
  )
  ...
  zetamat_g1 <- (diag(2 * 2) - kronecker(phimat_g1, phimat_g1)) %*%
    c(psimat) |>
    matrix(ncol = 2)
  zeta1_g1_pop <- zetamat_g1[1, 1]
  zeta2_g1_pop <- zetamat_g1[2, 2]
  zeta12_g1_pop <- zetamat_g1[1, 2]
  zetamat_g2 <- (diag(2 * 2) - kronecker(phimat_g2, phimat_g2)) %*%
    c(psimat) |>
    matrix(ncol = 2)
  ...
  if (nature == "categorical") {
    MM_grouping <- c(
      rep("subgroupA", 32 * 0.25),
      rep("subgroupB", 32 * 0.75),
      rep("subgroupA", 32 * 0.75),
      rep("subgroupB", 32 * 0.25)
    )
  }
  ...
  for (i in 1:64) {
    ...
    mu_i <- c(rnorm(1, mu[1], 1), rnorm(1, mu[2], 1))
    ...
    data_i <- t(tau_i + lambda_i %*% t(eta_i[, c("eta1", "eta2")])) +
      epsilon_i |>
        as.data.frame()
    ...
    data <- dplyr::full_join(
      data,
      data_i,
      by = join_by(id, obs, group, v1, v2, v3, v4, v5, v6, v7, v8)
    )
  ...
  partial_noninvariances <- list(
    c("f1 =~ v3", "f1 =~ v4", "v3 ~ 1", "v4 ~ 1"),
    c("f2 =~ v7", "f2 =~ v8", "v7 ~ 1", "v8 ~ 1")
  )

  #### Person-specific ####
  output_step1_personspec <- run_step1(
    data = data,
  ...
  step1_personspec_warning <- ifelse(
    is_empty(output_step1_personspec$warnings),
    FALSE,
    TRUE
  )
  step1_personspec_warning_text <- ifelse(
    is_empty(output_step1_personspec$warnings),
    "",
    paste(c(output_step1_personspec$warnings), collapse = "; ")
  )
  step1_personspec_error <- ifelse(
    is_empty(output_step1_personspec$result$error),
    FALSE,
    TRUE
  )
  step1_personspec_error_text <- ifelse(
    is_empty(output_step1_personspec$result$error),
    "",
    paste(c(output_step1_personspec$result$error), collapse = "; ")
  )

  #### Step 2 ####
  if (!step1_personspec_error) {
    # only proceed if there is no error in step 1
    output_step2_personspec <- run_step2(
      step1output = output_step1_personspec$result$result
    )
    # extract error/warning messages (if applicable):
    step2_personspec_warning <- ifelse(
      is_empty(output_step2_personspec$warnings),
      FALSE,
      TRUE
    )
    step2_personspec_warning_text <- ifelse(
      is_empty(output_step2_personspec$warnings),
      "",
      paste(c(output_step2_personspec$warnings), collapse = "; ")
    )
    step2_personspec_error <- ifelse(
      is_empty(output_step2_personspec$result$error),
      FALSE,
      TRUE
    )
    step2_personspec_error_text <- ifelse(
      is_empty(output_step2_personspec$result$error),
      "",
      paste(c(output_step2_personspec$result$error), collapse = "; ")
    )
  } else {
  ...
  if (
    grepl(
      ...
      step1_personspec_warning_text,
      ignore.case = TRUE
    ) &
      !step2_personspec_error
  ) {
    # if there are negative variances, remove those persons with negative variances and re-reun step 1 and step 2
    which_negative_theta <- which(sapply(
      output_step2_personspec$result$result$MMparameters$theta_group,
      function(mat) any(diag(mat) < 0)
    ))
    which_negative_psi <- which(sapply(
      output_step2_personspec$result$result$MMparameters$psi_group,
      function(mat) any(diag(mat) < 0)
    ))
  ...
  #### Step 3 ####
  if (!step1_personspec_error & !step2_personspec_error) {
    # only proceed if there is no error in step 1 as well as step 2
    output_step3_personspec <- run_step3(
      step2output = output_step2_personspec$result$result,
      id = "id",
      step3group = "group"
    )

    # extract error/warning messages (if applicable):
    step3_personspec_warning <- ifelse(
      is_empty(output_step3_personspec$warnings),
      FALSE,
      TRUE
    )
    step3_personspec_warning_text <- ifelse(
      is_empty(output_step3_personspec$warnings),
      "",
      paste(c(output_step3_personspec$warnings), collapse = "; ")
    )
    step3_personspec_error <- ifelse(
      is_empty(output_step3_personspec$result$error),
      FALSE,
      TRUE
    )
    step3_personspec_error_text <- ifelse(
      is_empty(output_step3_personspec$result$error),
      "",
      paste(c(output_step3_personspec$result$error), collapse = "; ")
    )
  ...
  if (
    !step1_personspec_error & !step2_personspec_error & !step3_personspec_error
  ) {
    ## SM parameters:
    estimates <- output_step3_personspec$result$result$estimates
    standarderrors <- setNames(
      summary(output_step3_personspec$result$result$model)$parameters$Std.Error,
  ...
  #### Group-specific ####
  output_step1_groupspec <- run_step1(
    data = data,
    measurementmodel = model_step1,
    group = "group",
    invariances = invariances,
  ...
  # extract error/warning messages (if applicable):
  step1_groupspec_warning <- ifelse(
    is_empty(output_step1_groupspec$warnings),
    FALSE,
    TRUE
  )
  step1_groupspec_warning_text <- ifelse(
    is_empty(output_step1_groupspec$warnings),
    "",
    paste(c(output_step1_groupspec$warnings), collapse = "; ")
  )
  step1_groupspec_error <- ifelse(
    is_empty(output_step1_groupspec$result$error),
    FALSE,
    TRUE
  )
  step1_groupspec_error_text <- ifelse(
    is_empty(output_step1_groupspec$result$error),
    "",
    paste(c(output_step1_groupspec$result$error), collapse = "; ")
  )
  ...
  #### Step 2 ####
  if (!step1_groupspec_error) {
    # only proceed if there is no error in step 1
    output_step2_groupspec <- run_step2(
      step1output = output_step1_groupspec$result$result
    )
    # extract error/warning messages (if applicable):
    step2_groupspec_warning <- ifelse(
      is_empty(output_step2_groupspec$warnings),
      FALSE,
      TRUE
    )
    step2_groupspec_warning_text <- ifelse(
      is_empty(output_step2_groupspec$warnings),
      "",
      paste(c(output_step2_groupspec$warnings), collapse = "; ")
    )
    step2_groupspec_error <- ifelse(
      is_empty(output_step2_groupspec$result$error),
      FALSE,
      TRUE
    )
    step2_groupspec_error_text <- ifelse(
      is_empty(output_step2_groupspec$result$error),
      "",
      paste(c(output_step2_groupspec$result$error), collapse = "; ")
    )
  } else {
  ...
  #### Step 3 ####
  if (!step1_groupspec_error & !step2_groupspec_error) {
    # only proceed if there is no error in step 1 as well as step 2
    output_step3_groupspec <- run_step3(
      step2output = output_step2_groupspec$result$result,
      id = "id",
      step3group = "group"
    )

    # extract error/warning messages (if applicable):
    step3_groupspec_warning <- ifelse(
      is_empty(output_step3_groupspec$warnings),
      FALSE,
      TRUE
    )
    step3_groupspec_warning_text <- ifelse(
      is_empty(output_step3_groupspec$warnings),
      "",
      paste(c(output_step3_groupspec$warnings), collapse = "; ")
    )
    step3_groupspec_error <- ifelse(
      is_empty(output_step3_groupspec$result$error),
      FALSE,
      TRUE
    )
    step3_groupspec_error_text <- ifelse(
      is_empty(output_step3_groupspec$result$error),
      "",
      paste(c(output_step3_groupspec$result$error), collapse = "; ")
    )
  ...
  if (
    !step1_groupspec_error & !step2_groupspec_error & !step3_groupspec_error
  ) {
    estimates <- output_step3_groupspec$result$result$estimates
    standarderrors <- setNames(
      summary(output_step3_groupspec$result$result$model)$parameters$Std.Error,
  ...
  output <- c(
    ...
    "nature" = nature,
    "duration" = duration,
    "phi11_g1_pop" = phi11_g1_pop,
    "phi12_g1_pop" = phi12_g1_pop,
    "phi21_g1_pop" = phi21_g1_pop,
    "phi22_g1_pop" = phi22_g1_pop,
    "phi11_g2_pop" = phi11_g2_pop,
    "phi12_g2_pop" = phi12_g2_pop,
    "phi21_g2_pop" = phi21_g2_pop,
    "phi22_g2_pop" = phi22_g2_pop,
    "zeta1_g1_pop" = zeta1_g1_pop,
    "zeta2_g1_pop" = zeta2_g1_pop,
    "zeta12_g1_pop" = zeta12_g1_pop,
    "zeta1_g2_pop" = zeta1_g2_pop,
    "zeta2_g2_pop" = zeta2_g2_pop,
    "zeta12_g2_pop" = zeta12_g2_pop,
    "personspec_phi11_g1" = personspec_phi11_g1,
    "personspec_phi12_g1" = personspec_phi12_g1,
    "personspec_phi21_g1" = personspec_phi21_g1,
    "personspec_phi22_g1" = personspec_phi22_g1,
    "personspec_phi11_g2" = personspec_phi11_g2,
    "personspec_phi12_g2" = personspec_phi12_g2,
    "personspec_phi21_g2" = personspec_phi21_g2,
    "personspec_phi22_g2" = personspec_phi22_g2,
    "personspec_zeta1_g1" = personspec_zeta1_g1,
    "personspec_zeta2_g1" = personspec_zeta2_g1,
    "personspec_zeta12_g1" = personspec_zeta12_g1,
    "personspec_zeta1_g2" = personspec_zeta1_g2,
    "personspec_zeta2_g2" = personspec_zeta2_g2,
    "personspec_zeta12_g2" = personspec_zeta12_g2,
    "personspec_phi11_g1_se" = personspec_phi11_g1_se,
    "personspec_phi12_g1_se" = personspec_phi12_g1_se,
    "personspec_phi21_g1_se" = personspec_phi21_g1_se,
    "personspec_phi22_g1_se" = personspec_phi22_g1_se,
    "personspec_phi11_g2_se" = personspec_phi11_g2_se,
    "personspec_phi12_g2_se" = personspec_phi12_g2_se,
    "personspec_phi21_g2_se" = personspec_phi21_g2_se,
    "personspec_phi22_g2_se" = personspec_phi22_g2_se,
    "personspec_zeta1_g1_se" = personspec_zeta1_g1_se,
    "personspec_zeta2_g1_se" = personspec_zeta2_g1_se,
    "personspec_zeta12_g1_se" = personspec_zeta12_g1_se,
    "personspec_zeta1_g2_se" = personspec_zeta1_g2_se,
    "personspec_zeta2_g2_se" = personspec_zeta2_g2_se,
    "personspec_zeta12_g2_se" = personspec_zeta12_g2_se,
    "groupspec_phi11_g1" = groupspec_phi11_g1,
    "groupspec_phi12_g1" = groupspec_phi12_g1,
    "groupspec_phi21_g1" = groupspec_phi21_g1,
    "groupspec_phi22_g1" = groupspec_phi22_g1,
    "groupspec_phi11_g2" = groupspec_phi11_g2,
    "groupspec_phi12_g2" = groupspec_phi12_g2,
    "groupspec_phi21_g2" = groupspec_phi21_g2,
    "groupspec_phi22_g2" = groupspec_phi22_g2,
    "groupspec_zeta1_g1" = groupspec_zeta1_g1,
    "groupspec_zeta2_g1" = groupspec_zeta2_g1,
    "groupspec_zeta12_g1" = groupspec_zeta12_g1,
    "groupspec_zeta1_g2" = groupspec_zeta1_g2,
    "groupspec_zeta2_g2" = groupspec_zeta2_g2,
    "groupspec_zeta12_g2" = groupspec_zeta12_g2,
    "groupspec_phi11_g1_se" = groupspec_phi11_g1_se,
    "groupspec_phi12_g1_se" = groupspec_phi12_g1_se,
    "groupspec_phi21_g1_se" = groupspec_phi21_g1_se,
    "groupspec_phi22_g1_se" = groupspec_phi22_g1_se,
    "groupspec_phi11_g2_se" = groupspec_phi11_g2_se,
    "groupspec_phi12_g2_se" = groupspec_phi12_g2_se,
    "groupspec_phi21_g2_se" = groupspec_phi21_g2_se,
    "groupspec_phi22_g2_se" = groupspec_phi22_g2_se,
    "groupspec_zeta1_g1_se" = groupspec_zeta1_g1_se,
    "groupspec_zeta2_g1_se" = groupspec_zeta2_g1_se,
    "groupspec_zeta12_g1_se" = groupspec_zeta12_g1_se,
    "groupspec_zeta1_g2_se" = groupspec_zeta1_g2_se,
    "groupspec_zeta2_g2_se" = groupspec_zeta2_g2_se,
    "groupspec_zeta12_g2_se" = groupspec_zeta12_g2_se,
    "bias_lambda3" = bias_lambda3,
    "bias_lambda4" = bias_lambda4,
    "bias_lambda7" = bias_lambda7,
    "bias_lambda8" = bias_lambda8,
    "bias_tau3" = bias_tau3,
    "bias_tau4" = bias_tau4,
    "bias_tau7" = bias_tau7,
    "bias_tau8" = bias_tau8,
    "bias_theta1" = bias_theta1,
    "bias_theta2" = bias_theta2,
    "bias_theta3" = bias_theta3,
    "bias_theta4" = bias_theta4,
    "bias_theta5" = bias_theta5,
    "bias_theta6" = bias_theta6,
    "bias_theta7" = bias_theta7,
    "bias_theta8" = bias_theta8,
    "RMSE_lambda3" = RMSE_lambda3,
    "RMSE_lambda4" = RMSE_lambda4,
    "RMSE_lambda7" = RMSE_lambda7,
    "RMSE_lambda8" = RMSE_lambda8,
    "RMSE_tau3" = RMSE_tau3,
    "RMSE_tau4" = RMSE_tau4,
    "RMSE_tau7" = RMSE_tau7,
    "RMSE_tau8" = RMSE_tau8,
    ...
    "RMSE_theta4" = RMSE_theta4,
    "RMSE_theta5" = RMSE_theta5,
    "RMSE_theta6" = RMSE_theta6,
    "RMSE_theta7" = RMSE_theta7,
    "RMSE_theta8" = RMSE_theta8,
    "step1_personspec_warning" = step1_personspec_warning,
    "step2_personspec_warning" = step2_personspec_warning,
    "step3_personspec_warning" = step3_personspec_warning,
    "step1_personspec_error" = step1_personspec_error,
    "step2_personspec_error" = step2_personspec_error,
    "step3_personspec_error" = step3_personspec_error,
    "n_negvars_personspec" = n_negvars_personspec,
    "step1_groupspec_warning" = step1_groupspec_warning,
    "step2_groupspec_warning" = step2_groupspec_warning,
    "step3_groupspec_warning" = step3_groupspec_warning,
    "step1_groupspec_error" = step1_groupspec_error,
    "step2_groupspec_error" = step2_groupspec_error,
    "step3_groupspec_error" = step3_groupspec_error,
    "rerun_step1_groupspec" = rerun_step1_groupspec,
    "seed" = seed_cond,
    "pos" = pos,
    "step1_personspec_warning_text" = step1_personspec_warning_text,
    "step2_personspec_warning_text" = step2_personspec_warning_text,
    "step3_personspec_warning_text" = step3_personspec_warning_text,
    "step1_personspec_error_text" = step1_personspec_error_text,
    "step2_personspec_error_text" = step2_personspec_error_text,
    "step3_personspec_error_text" = step3_personspec_error_text,
    "step1_groupspec_warning_text" = step1_groupspec_warning_text,
    "step2_groupspec_warning_text" = step2_groupspec_warning_text,
    "step3_groupspec_warning_text" = step3_groupspec_warning_text,
    "step1_groupspec_error_text" = step1_groupspec_error_text,
    "step2_groupspec_error_text" = step2_groupspec_error_text,
    "step3_groupspec_error_text" = step3_groupspec_error_text
  )
