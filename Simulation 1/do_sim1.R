...
do_sim1 <- function(pos, cond, outputfile, verbose = FALSE) {
  ...
  # residual variances:
  if (
    invariance_level %in% c("full_scalar", "partial_scalar", "partial_metric")
  ) {
    if (pattern == "mixed") {
  ...
  lambdastar_g1_pop <- diag(
    psimat %*% t(lambda_g1) %*% solve(sigma_g1) %*% lambda_g1
  )
  lambdastar_g2_pop <- diag(
    psimat %*% t(lambda_g2) %*% solve(sigma_g2) %*% lambda_g2
  )
  ...
  for (g in c("group1", "group2")) {
    ...
    for (i in ids_g) {
      ...
      mu_i <- c(rnorm(1, mu_g[1], 1), rnorm(1, mu_g[2], 1))
    ...
    data_g <- t(tau_g + lambda_g %*% t(eta_g[, c("eta1", "eta2")])) +
      epsilon_g |>
        as.data.frame()
  ...
  step1_single_warning <- ifelse(
    is_empty(output_step1_single$warnings),
    FALSE,
    TRUE
  )
  step1_single_warning_text <- ifelse(
    is_empty(output_step1_single$warnings),
    "",
    paste(c(output_step1_single$warnings), collapse = "; ")
  )
  step1_single_error <- ifelse(
    is_empty(output_step1_single$result$error),
    FALSE,
    TRUE
  )
  step1_single_error_text <- ifelse(
    is_empty(output_step1_single$result$error),
    "",
    paste(c(output_step1_single$result$error), collapse = "; ")
  )
  ...
  #### Step 2 ####
  if (!step1_single_error) {
    # only proceed if there is no error in step 1
    output_step2_single <- run_step2(
      step1output = output_step1_single$result$result
    )
    # extract error/warning messages (if applicable):
    step2_single_warning <- ifelse(
      is_empty(output_step2_single$warnings),
      FALSE,
      TRUE
    )
    step2_single_warning_text <- ifelse(
      is_empty(output_step2_single$warnings),
      "",
      paste(c(output_step2_single$warnings), collapse = "; ")
    )
    step2_single_error <- ifelse(
      is_empty(output_step2_single$result$error),
      FALSE,
      TRUE
    )
    step2_single_error_text <- ifelse(
      is_empty(output_step2_single$result$error),
      "",
      paste(c(output_step2_single$result$error), collapse = "; ")
    )
  } else {
  ...
  #### Step 3 ####
  if (!step1_single_error & !step2_single_error) {
    # only proceed if there is no error in step 1 as well as step 2
    output_step3_single <- run_step3(
      step2output = output_step2_single$result$result,
      id = "id",
      step3group = "group"
    )

    # extract error/warning messages (if applicable):
    step3_single_warning <- ifelse(
      is_empty(output_step3_single$warnings),
      FALSE,
      TRUE
    )
    step3_single_warning_text <- ifelse(
      is_empty(output_step3_single$warnings),
      "",
      paste(c(output_step3_single$warnings), collapse = "; ")
    )
    step3_single_error <- ifelse(
      is_empty(output_step3_single$result$error),
      FALSE,
      TRUE
    )
    step3_single_error_text <- ifelse(
      is_empty(output_step3_single$result$error),
      "",
      paste(c(output_step3_single$result$error), collapse = "; ")
    )
  ...
  if (!step1_single_error & !step2_single_error & !step3_single_error) {
    ...
    standarderrors <- setNames(
      summary(output_step3_single$result$result$model)$parameters$Std.Error,
  ...
  if (invariance_level == "partial_scalar") {
    partial_noninvariances <- list(c("v3 ~ 1", "v4 ~ 1"), c("v7 ~ 1", "v8 ~ 1"))
  }
  if (invariance_level == "partial_metric") {
  ...
  # extract error/warning messages (if applicable):
  step1_multi_warning <- ifelse(
    is_empty(output_step1_multi$warnings),
    FALSE,
    TRUE
  )
  step1_multi_warning_text <- ifelse(
    is_empty(output_step1_multi$warnings),
    "",
    paste(c(output_step1_multi$warnings), collapse = "; ")
  )
  step1_multi_error <- ifelse(
    is_empty(output_step1_multi$result$error),
    FALSE,
    TRUE
  )
  step1_multi_error_text <- ifelse(
    is_empty(output_step1_multi$result$error),
    "",
    paste(c(output_step1_multi$result$error), collapse = "; ")
  )
  ...
  #### Step 2 ####
  if (!step1_multi_error) {
    # only proceed if there is no error in step 1
    output_step2_multi <- run_step2(
      step1output = output_step1_multi$result$result
    )
    # extract error/warning messages (if applicable):
    step2_multi_warning <- ifelse(
      is_empty(output_step2_multi$warnings),
      FALSE,
      TRUE
    )
    step2_multi_warning_text <- ifelse(
      is_empty(output_step2_multi$warnings),
      "",
      paste(c(output_step2_multi$warnings), collapse = "; ")
    )
    step2_multi_error <- ifelse(
      is_empty(output_step2_multi$result$error),
      FALSE,
      TRUE
    )
    step2_multi_error_text <- ifelse(
      is_empty(output_step2_multi$result$error),
      "",
      paste(c(output_step2_multi$result$error), collapse = "; ")
    )
  } else {
  ...
  #### Step 3 ####
  if (!step1_multi_error & !step2_multi_error) {
    # only proceed if there is no error in step 1 as well as step 2
    output_step3_multi <- run_step3(
      step2output = output_step2_multi$result$result,
      id = "id",
      step3group = "group"
    )

    # extract error/warning messages (if applicable):
    step3_multi_warning <- ifelse(
      is_empty(output_step3_multi$warnings),
      FALSE,
      TRUE
    )
    step3_multi_warning_text <- ifelse(
      is_empty(output_step3_multi$warnings),
      "",
      paste(c(output_step3_multi$warnings), collapse = "; ")
    )
    step3_multi_error <- ifelse(
      is_empty(output_step3_multi$result$error),
      FALSE,
      TRUE
    )
    step3_multi_error_text <- ifelse(
      is_empty(output_step3_multi$result$error),
      "",
      paste(c(output_step3_multi$result$error), collapse = "; ")
    )
  ...
  if (!step1_multi_error & !step2_multi_error & !step3_multi_error) {
    ...
    standarderrors <- setNames(
      summary(output_step3_multi$result$result$model)$parameters$Std.Error,
    ...
    tau_true <- matrix(
      c(tau_g1[c(3, 4, 7, 8)], tau_g2[c(3, 4, 7, 8)]),
      nrow = 2,
      byrow = TRUE
    )
  ...
  output <- c(
    ...
    "single_phi11_g1" = single_phi11_g1,
    "single_phi12_g1" = single_phi12_g1,
    "single_phi21_g1" = single_phi21_g1,
    "single_phi22_g1" = single_phi22_g1,
    "single_phi11_g2" = single_phi11_g2,
    "single_phi12_g2" = single_phi12_g2,
    "single_phi21_g2" = single_phi21_g2,
    "single_phi22_g2" = single_phi22_g2,
    "single_zeta1_g1" = single_zeta1_g1,
    "single_zeta2_g1" = single_zeta2_g1,
    "single_zeta12_g1" = single_zeta12_g1,
    "single_zeta1_g2" = single_zeta1_g2,
    "single_zeta2_g2" = single_zeta2_g2,
    "single_zeta12_g2" = single_zeta12_g2,
    "single_phi11_g1_se" = single_phi11_g1_se,
    "single_phi12_g1_se" = single_phi12_g1_se,
    "single_phi21_g1_se" = single_phi21_g1_se,
    "single_phi22_g1_se" = single_phi22_g1_se,
    "single_phi11_g2_se" = single_phi11_g2_se,
    "single_phi12_g2_se" = single_phi12_g2_se,
    "single_phi21_g2_se" = single_phi21_g2_se,
    "single_phi22_g2_se" = single_phi22_g2_se,
    "single_zeta1_g1_se" = single_zeta1_g1_se,
    "single_zeta2_g1_se" = single_zeta2_g1_se,
    "single_zeta12_g1_se" = single_zeta12_g1_se,
    "single_zeta1_g2_se" = single_zeta1_g2_se,
    "single_zeta2_g2_se" = single_zeta2_g2_se,
    "single_zeta12_g2_se" = single_zeta12_g2_se,
    "multi_phi11_g1" = multi_phi11_g1,
    "multi_phi12_g1" = multi_phi12_g1,
    "multi_phi21_g1" = multi_phi21_g1,
    "multi_phi22_g1" = multi_phi22_g1,
    "multi_phi11_g2" = multi_phi11_g2,
    "multi_phi12_g2" = multi_phi12_g2,
    "multi_phi21_g2" = multi_phi21_g2,
    "multi_phi22_g2" = multi_phi22_g2,
    "multi_zeta1_g1" = multi_zeta1_g1,
    "multi_zeta2_g1" = multi_zeta2_g1,
    "multi_zeta12_g1" = multi_zeta12_g1,
    "multi_zeta1_g2" = multi_zeta1_g2,
    "multi_zeta2_g2" = multi_zeta2_g2,
    "multi_zeta12_g2" = multi_zeta12_g2,
    "multi_phi11_g1_se" = multi_phi11_g1_se,
    "multi_phi12_g1_se" = multi_phi12_g1_se,
    "multi_phi21_g1_se" = multi_phi21_g1_se,
    "multi_phi22_g1_se" = multi_phi22_g1_se,
    "multi_phi11_g2_se" = multi_phi11_g2_se,
    "multi_phi12_g2_se" = multi_phi12_g2_se,
    "multi_phi21_g2_se" = multi_phi21_g2_se,
    "multi_phi22_g2_se" = multi_phi22_g2_se,
    "multi_zeta1_g1_se" = multi_zeta1_g1_se,
    "multi_zeta2_g1_se" = multi_zeta2_g1_se,
    "multi_zeta12_g1_se" = multi_zeta12_g1_se,
    "multi_zeta1_g2_se" = multi_zeta1_g2_se,
    "multi_zeta2_g2_se" = multi_zeta2_g2_se,
    "multi_zeta12_g2_se" = multi_zeta12_g2_se,
    "lambdastar_f1_g1_pop" = lambdastar_f1_g1_pop,
    "lambdastar_f2_g1_pop" = lambdastar_f2_g1_pop,
    "lambdastar_f1_g2_pop" = lambdastar_f1_g2_pop,
    "lambdastar_f2_g2_pop" = lambdastar_f2_g2_pop,
    "lambdastar_f1_g1" = lambdastar_f1_g1,
    "lambdastar_f2_g1" = lambdastar_f2_g1,
    "lambdastar_f1_g2" = lambdastar_f1_g2,
    "lambdastar_f2_g2" = lambdastar_f2_g2,
    ...
    "RMSE_tau" = RMSE_tau,
    "step1_single_warning" = step1_single_warning,
    "step2_single_warning" = step2_single_warning,
    "step3_single_warning" = step3_single_warning,
    "step1_single_error" = step1_single_error,
    "step2_single_error" = step2_single_error,
    "step3_single_error" = step3_single_error,
    "rerun_step1_single" = rerun_step1_single,
    "step1_multi_warning" = step1_multi_warning,
    "step2_multi_warning" = step2_multi_warning,
    "step3_multi_warning" = step3_multi_warning,
    "step1_multi_error" = step1_multi_error,
    "step2_multi_error" = step2_multi_error,
    "step3_multi_error" = step3_multi_error,
    "rerun_step1_multi" = rerun_step1_multi,
    "seed" = seed_cond,
    "pos" = pos,
    "step1_single_warning_text" = step1_single_warning_text,
    "step2_single_warning_text" = step2_single_warning_text,
    "step3_single_warning_text" = step3_single_warning_text,
    "step1_single_error_text" = step1_single_error_text,
    "step2_single_error_text" = step2_single_error_text,
    "step3_single_error_text" = step3_single_error_text,
    "step1_multi_warning_text" = step1_multi_warning_text,
    "step2_multi_warning_text" = step2_multi_warning_text,
    "step3_multi_warning_text" = step3_multi_warning_text,
    "step1_multi_error_text" = step1_multi_error_text,
    "step2_multi_error_text" = step2_multi_error_text,
    "step3_multi_error_text" = step3_multi_error_text
  )
