step2 <- function(step1output) {
  ...
  for (m in 1:M) {
    ...
    data <- dplyr::full_join(
      data,
      temp,
      by = c(group, indicators[indicators %in% colnames(temp)])
    )
  }
  ...
  if (purrr::is_empty(group)) {
    ...
    psi_block <- alpha_block <- lambda_block <- theta_block <- tau_block <- vector(
      mode = "list",
      length = M
    )
    for (m in 1:M) {
  ...
  if (!purrr::is_empty(group)) {
    ...
    # column names = names of the factors
    if (is.list(fit_step1)) {
      rownames(lambda_star) <- rownames(theta_star) <- lavInspect(
        fit_step1[[1]],
        "group.label"
      )
    } else {
    ...
    psi_group <- alpha_group <- lambda_group <- theta_group <- tau_group <- vector(
      mode = "list",
      length = G
    )
    for (g in 1:G) {
